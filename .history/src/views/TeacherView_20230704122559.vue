  ...
  methods: {
    ...
    handleSearch(results) {
       ...
       if (results.data.length > 0) {
        // 根据搜索的关键词进行筛选
       // this.searchResults = this.questionList.filter(question => {
          // 这里可以根据题目名称或其他属性进行筛选
         // return question.questionName.includes(results);
        //});
        console.log('searchResults')
        const searchList=results.data.data
        console.log(searchList)
			    this.searchResults=searchList.map(question=>({
            questionName:question.questionName,
            questionId:question.questionId
          }))
        console.log(searchResults)
        this.showSearchResults = true; // 显示搜索结果
       } else {
    ...
    handleDialogConfirm() {
      ...
      this.$axios({
           method:'post',
           url:'http://43.142.234.223:8080/question', 
            headers:{
              'token':localStorage.getItem("token")
            },
           data:{
    ...
    deleteQuestion(index) {
      this.$axios
        .delete(`/question/${this.questionId}`,{
          headers:{
              'token':localStorage.getItem("token")
            }
        })
