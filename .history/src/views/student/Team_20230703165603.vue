<template>
  ...
  <tbody>
    <tr v-for="team in teams" :key="team.teamId">
      <td>{{ team.teamName }}</td>
      <td>{{ team.questionName }}</td>
      <td>
        <button @click="joinTeam(team.teamId)">加入组队</button>
        <button @click="viewTeamDocument(team.teamId)">查看队伍文档</button>
        <button @click="downloadFile(team.filePath)">下载文件</button>
      </td>
    </tr>
  </tbody>
</table>
