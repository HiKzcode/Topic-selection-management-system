<template>
  <div>
    ...
    <div v-else-if="currentView === 'createTeam'">
     ...
     <el-select v-model="selectedQuestionId" placeholder="请选择题目" @remote="getquestionlist" @change="updateTeamInfo" style="margin-right: 400px; width: 220px;">
        <el-option
       ...
       ></el-option>
		...
		<el-input v-model="selectedQuestionName"></el-input>
</el-form-item>
...
,     myteamo:{},
	...
	computed: {
    questionNameById() {
      const selectedQuestion = this.questionlist.find(question => question.questionId === this.selectedQuestionId);
      if (selectedQuestion) {
        return selectedQuestion.questionName;
      }
      return '';
    },
  },
  methods: {
      ...
      if (this.selectedQuestion) {
        this.cForm.questionName = this.selectedQuestion.questionName;
				this.cForm.questionId=this.selectedQuestion.questionId;
      } else {
        this.cForm.questionName = '';
				this.cForm.questionId='';
      }
    },
    uploadFile() {
    ...
    //创建队伍的信息提交
		onSubmit() {
			const name=this.questionNameById
       ...
       this.$axios({
           ...
           'token':localStorage.getItem("token")
				 ...
				 data:{
					 ...
					 questionName:this.questionNameById,
